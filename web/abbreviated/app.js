...
function connectToServer() {
    let url = serverUrlInput.value || "ws://127.0.0.1:8088/xiaozhi/v1/";
...
async function sendHelloMessage() {
    ...
    try {
        ...
        // Get actual audio settings from microphone
        ...
        // Calculate frame duration based on our buffer size (960 samples)
        const actualSampleRate = settings.sampleRate || 16000;
        const frameDuration = (960 / actualSampleRate) * 1000; // Convert to milliseconds
        log(`实际采样率: ${actualSampleRate}Hz, 帧时长: ${frameDuration.toFixed(2)}ms`, 'info');

        // Create hello message with audio parameters
        const helloMessage = {
            type: 'hello',
            version: 1,
            transport: 'websocket',
            ...
            audio_params: {
                ...
                frame_duration: Math.round(frameDuration) // Round to nearest millisecond
            }
        };
